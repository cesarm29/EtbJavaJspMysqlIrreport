...
import DTO.ModernizacionLlanosDTO;
import bd.Conectar;
 ...
 * @author cesar1
 */
public class ModernizacionLlanosDAO {
    
PreparedStatement ps = null;
ResultSet rs = null;
ModernizacionLlanosDTO objlogin = new ModernizacionLlanosDTO();
ArrayList<ModernizacionLlanosDTO> ArregloUsuario = new ArrayList<ModernizacionLlanosDTO>(); 
private int rol;
private int opcion = 0; 

    public ModernizacionLlanosDAO() {
    }

//consulta de datos, para hacer la busqueda se pasa el parametro pr     
public ModernizacionLlanosDTO getConsulta(int pr) throws SQLException, NamingException, Exception {
        
        ModernizacionLlanosDTO contdto = null;
        Connection cnn = Conectar.getInstace();
        ps = cnn.prepareStatement("SELECT * FROM modernizacionllanos WHERE (pr = ? )");
        ps.setInt(1, pr);
        ResultSet resul = ps.executeQuery();
        while (resul.next()) {
            contdto = new ModernizacionLlanosDTO();
            ...
            contdto.setIdMODERNIZACIONLLANOS(pr);
        }
        return contdto;
    }

//consulta de datos, para hacer la busqueda se pasa el parametro id    
public ModernizacionLlanosDTO getPorID(int idMODERNIZACIONLLANOS) throws SQLException, NamingException, Exception {
        
        ModernizacionLlanosDTO contdto = null;
        Connection cnn = Conectar.getInstace();
        ps = cnn.prepareStatement("SELECT * FROM modernizacionllanos WHERE (idMODERNIZACIONLLANOS = ? )");
        ps.setInt(1, idMODERNIZACIONLLANOS);
        ResultSet resul = ps.executeQuery();
        while (resul.next()) {
            contdto = new ModernizacionLlanosDTO();
            contdto.setCentral(resul.getString("central"));
            ...
            contdto.setContratista(resul.getString("contratista"));
            contdto.setUSUARIOS_idUSUARIOS(resul.getInt("USUARIOS_idUSUARIOS"));
            contdto.setIdMODERNIZACIONLLANOS(idMODERNIZACIONLLANOS);
        }
        return contdto;
    }

//crea registro modernizacion llanos
public String crearModernizacionLlanos(ModernizacionLlanosDTO contrdto) throws Exception {

        String Respuesta = "";
        Connection cnn = Conectar.getInstace();
        
        try{
            ps = cnn.prepareStatement("INSERT INTO `modernizacionllanos` (`central`, `distrito`, `pr`, `tema`, `costocontrato`, `costoproyecto`, `pares`, `estado`, `fechaentrega`, `inicioprogramado`, `finprogramado`, `avance`, `fechareporte`, `mesfacturacion`, `contratista`, `USUARIOS_idUSUARIOS`)"
                    + " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
...
//modifica registro modernizacion llanos
public String ModificarModernizacionLlanos(ModernizacionLlanosDTO contrdto) throws Exception {

        String Respuesta = "";
        Connection cnn = Conectar.getInstace();
        
        try{
            ps = cnn.prepareStatement("UPDATE modernizacionllanos SET central = ?, distrito = ?, pr = ?, tema = ?, costocontrato = ?, costoproyecto = ?, pares = ?, estado = ?, fechaentrega = ?, inicioprogramado = ?, finprogramado = ?, avance = ?, fechareporte = ?, mesfacturacion = ?, contratista = ?, USUARIOS_idUSUARIOS = ?   WHERE (idMODERNIZACIONLLANOS = ?)");
            ...
            ps.setString(15, contrdto.getContratista());
            ps.setInt(16, contrdto.getUSUARIOS_idUSUARIOS());
            ps.setInt(17, contrdto.getIdMODERNIZACIONLLANOS());
...
//elimina registro modernizacion llanos
public String EliminarModernizacionLlanos(ModernizacionLlanosDTO contrdto) throws Exception {
        
        String Respuesta = "";
        Connection cnn = Conectar.getInstace();
        try{
            ps = cnn.prepareStatement("delete from modernizacionllanos  where idMODERNIZACIONLLANOS= " + contrdto.getIdMODERNIZACIONLLANOS());
...
//muestra datos en tabla, para la busqueda se pasa el parametro filtro
public ResultSet getTabla(String filtro) throws SQLException, NamingException, Exception {    
        Connection cnn = Conectar.getInstace();     
        String sql = "SELECT * FROM modernizacionllanos where  pr  like '%"+ filtro+"%'";
        return cnn.createStatement().executeQuery(sql);
    }

//muestra datos en tabla, para la busqueda se pasa el parametro filtro, para hacer el conteo de las paginas se usa el parametro page
public ResultSet getTabla2(String filtro, int page) throws Exception {
        ...
        String sql = "SELECT SQL_CALC_FOUND_ROWS * FROM modernizacionllanos "+fil+ limit;
        return cnn.createStatement().executeQuery(sql);
    }
    //cuenta el total de registros
    public int getTotalRegistro(Boolean filtrado)throws Exception{
        String sql = "select count(central)as t from modernizacionllanos";
...
public Boolean Exists(String idMODERNIZACIONLLANOS) throws Exception{
            
        Connection cnn = Conectar.getInstace();
        ResultSet r =  cnn.createStatement().executeQuery("SELECT * FROM modernizacionllanos where idMODERNIZACIONLLANOS="+idMODERNIZACIONLLANOS);
...
public ArrayList<ModernizacionLlanosDTO>  ListarTodos ()throws Exception{    
       Connection cnn = Conectar.getInstace();
       try {
            ps=cnn.prepareStatement("SELECT * FROM  modernizacionllanos");
            rs = ps.executeQuery();
            while (rs.next()){
            ModernizacionLlanosDTO objl = new ModernizacionLlanosDTO();
            objl.setIdMODERNIZACIONLLANOS(rs.getInt("idMODERNIZACIONLLANOS"));

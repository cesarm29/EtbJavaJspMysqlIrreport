 ...
 * @author cesar1
 */
public class MicromotoresDAO {

    PreparedStatement ps = null;
    ResultSet rs = null;
    MicromotoresDTO objlogin = new MicromotoresDTO();
    ArrayList<MicromotoresDTO> ArregloUsuario = new ArrayList<MicromotoresDTO>();
    private int rol;
    private int opcion = 0;

    public MicromotoresDAO() {
    }
//consulta datos por pr
    public MicromotoresDTO getConsulta(int pr) throws Exception {

        MicromotoresDTO contdto = null;
        Connection cnn = Conectar.getInstace();
        ps = cnn.prepareStatement("SELECT * FROM micromotores WHERE (pr = ? )");
        ps.setInt(1, pr);
        ResultSet resul = ps.executeQuery();
        while (resul.next()) {

            contdto = new MicromotoresDTO();
            ...
            contdto.setIdMICROMOTORES(pr);
        }
        return contdto;
    }
//consulta datos por id
    public MicromotoresDTO getPorID(int idMICROMOTORES) throws SQLException, NamingException, Exception {

        MicromotoresDTO contdto = null;
        Connection cnn = Conectar.getInstace();
        ps = cnn.prepareStatement("SELECT * FROM micromotores WHERE (idMICROMOTORES = ? )");
        ps.setInt(1, idMICROMOTORES);
        ResultSet resul = ps.executeQuery();
        while (resul.next()) {
            contdto = new MicromotoresDTO();
            contdto.setCentral(resul.getString("central"));
            ...
            contdto.setContratista(resul.getString("contratista"));
            contdto.setUSUARIOS_idUSUARIOS(resul.getInt("USUARIOS_idUSUARIOS"));
            contdto.setIdMICROMOTORES(idMICROMOTORES);
        }
        return contdto;
    }
//crea registro micromotores
    public String CrearMicromotores(MicromotoresDTO contrdto) throws Exception {
        String Respuesta = "";
        Connection cnn = Conectar.getInstace();
        try {
            ps = cnn.prepareStatement("INSERT INTO `micromotores` (`central`, `distrito`, `pr`, `tema`, `costocontrato`, `costoproyecto`, `pares`, `estado`, `fechaentrega`, `inicioprogramado`, `finprogramado`, `avance`, `fechareporte`, `mesfacturacion`, `contratista`, `USUARIOS_idUSUARIOS`)"
                    + " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            ...
            if (result != 0) {
                Respuesta = "Tu Insersion De Micromotores Ha Sido Exitosa";
            } else {
                Respuesta = "Tu Insersion De Micromotores Ha Fallado";
            }
...
//modifica registro micromotores
    public String ModificarMicromotores(MicromotoresDTO contrdto) throws Exception {
        String Respuesta = "";
        Connection cnn = Conectar.getInstace();
        try {
            ps = cnn.prepareStatement("UPDATE micromotores SET central = ?, distrito = ?, pr = ?, tema = ?, costocontrato = ?, costoproyecto = ?, pares = ?, estado = ?, fechaentrega = ?, inicioprogramado = ?, finprogramado = ?, avance = ?, fechareporte = ?, mesfacturacion = ?, contratista = ?, USUARIOS_idUSUARIOS = ?   WHERE (idMICROMOTORES = ?)");
            ps.setString(1, contrdto.getCentral());
            ps.setInt(2, contrdto.getDistrito());
            ps.setInt(3, contrdto.getPr());
            ps.setString(4, contrdto.getTema());
            ...
            ps.setString(15, contrdto.getContratista());
            ps.setInt(16, contrdto.getUSUARIOS_idUSUARIOS());
            ps.setInt(17, contrdto.getIdMICROMOTORES());

            int result = ps.executeUpdate();
            if (result != 0) {
                Respuesta = "Tu Modificacion De Micromotores Ha Sido Exitosa";
            } else {
                Respuesta = "Tu Modificacion De Micromotores Ha Fallado ";
            }
        } catch (SQLException ex) {
            Respuesta = ex.getMessage();
            Respuesta = "Error";
...
//elimina registro micromotores
    public String EliminarMicromotores(MicromotoresDTO contrdto) throws Exception {
        String Respuesta = "";
        Connection cnn = Conectar.getInstace();
        try {
            ps = cnn.prepareStatement("delete from micromotores  where idMICROMOTORES= " + contrdto.getIdMICROMOTORES());
            int result = ps.executeUpdate();
            if (result != 0) {
                Respuesta = "Tu Eliminacion De Micromotores Ha Sido Exitosa";
            } else {
                Respuesta = "Tu Eliminacion De Micromotores Ha Fallado";
            }
        } catch (SQLException ex) {
            Respuesta = ex.getMessage();
            Respuesta = "Error";
...
//muestra datos en una tabla, como parametro para hacer la busqueda se usa filtro
    public ResultSet getTabla(String filtro) throws SQLException, NamingException, Exception {
        Connection cnn = Conectar.getInstace();
        String sql = "SELECT * FROM micromotores where  pr  like '%" + filtro + "%'";
        return cnn.createStatement().executeQuery(sql);
    }
//muestra datos en una tabla, como parametro para hacer la busqueda se una filtro, para hacer el conteo de paginas se usa page    
    public ResultSet getTabla2(String filtro, int page) throws Exception {
        ...
        String sql = "SELECT SQL_CALC_FOUND_ROWS * FROM micromotores "+fil+ limit;
        return cnn.createStatement().executeQuery(sql);
    }
    //cuenta el total de registros
    public int getTotalRegistro(Boolean filtrado)throws Exception{
        String sql = "select count(central)as t from micromotores";
    ...
    public Boolean Exists(String idMICROMOTORES) throws Exception {

        Connection cnn = Conectar.getInstace();
        ResultSet r = cnn.createStatement().executeQuery("SELECT * FROM micromotores where idMICROMOTORES=" + idMICROMOTORES);
    ...
    public ArrayList<MicromotoresDTO> ListarTodos() throws Exception {
        Connection cnn = Conectar.getInstace();
        try {
            ps = cnn.prepareStatement("SELECT * FROM  micromotores");
            rs = ps.executeQuery();
            while (rs.next()) {
                MicromotoresDTO objl = new MicromotoresDTO();
                objl.setIdMICROMOTORES(rs.getInt("idMICROMOTORES"));

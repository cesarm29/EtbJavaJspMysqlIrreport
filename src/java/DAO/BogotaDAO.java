...
public class BogotaDAO {
    ...
    ResultSet rs = null;
    BogotaDTO objlogin = new BogotaDTO();
    ArrayList<BogotaDTO> ArregloUsuario = new ArrayList<BogotaDTO>();
    private int rol;
    private int opcion = 0;

    public BogotaDAO() {
    }
// consulta de datos, se pasa parametro pr
    public BogotaDTO getConsulta(int pr) throws Exception {

        BogotaDTO contdto = null;
        Connection cnn = Conectar.getInstace();
        ps = cnn.prepareStatement("SELECT * FROM bogota WHERE (pr = ? )");
        ps.setInt(1, pr);
        ResultSet resul = ps.executeQuery();
        while (resul.next()) {

            contdto = new BogotaDTO();
            ...
            contdto.setIdBOGOTA(pr);
        }
        return contdto;
    }
//consulta por id, se pasa parametro id
    public BogotaDTO getPorID(int idBOGOTA) throws SQLException, NamingException, Exception {


        BogotaDTO contdto = null;
        Connection cnn = Conectar.getInstace();
        ps = cnn.prepareStatement("SELECT * FROM bogota WHERE (idBOGOTA = ? )");
        ps.setInt(1, idBOGOTA);
        ResultSet resul = ps.executeQuery();
        while (resul.next()) {
            contdto = new BogotaDTO();
            contdto.setCentral(resul.getString("central"));
            ...
            contdto.setContratista(resul.getString("contratista"));
            contdto.setUSUARIOS_idUSUARIOS(resul.getInt("USUARIOS_idUSUARIOS"));
            contdto.setIdBOGOTA(idBOGOTA);
        }
        return contdto;
    }
//crea registro bogota
    public String crearBogota(BogotaDTO contrdto) throws Exception {
        String Respuesta = "";
        Connection cnn = Conectar.getInstace();
        try {
            ps = cnn.prepareStatement("INSERT INTO `bogota` (`central`, `distrito`, `pr`, `tema`, `costocontrato`, `costoproyecto`, `pares`, `estado`, `fechaentrega`, `inicioprogramado`, `finprogramado`, `avance`, `fechareporte`, `mesfacturacion`, `contratista`, `USUARIOS_idUSUARIOS`)"
                    + " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            ...
            if (result != 0) {
                Respuesta = "Tu Insersion De Bogota Ha Sido Exitosa";
            } else {
                Respuesta = "Tu Insersion De Bogota Ha Fallado";
            }
...
//modifica registro bogota
    public String ModificarBogota(BogotaDTO contrdto) throws Exception {
        String Respuesta = "";
        Connection cnn = Conectar.getInstace();
        try {
            ps = cnn.prepareStatement("UPDATE bogota SET central = ?, distrito = ?, pr = ?, tema = ?, costocontrato = ?, costoproyecto = ?, pares = ?, estado = ?, fechaentrega = ?, inicioprogramado = ?, finprogramado = ?, avance = ?, fechareporte = ?, mesfacturacion = ?, contratista = ?, USUARIOS_idUSUARIOS = ?   WHERE (idBOGOTA = ?)");
            ps.setString(1, contrdto.getCentral());
            ps.setInt(2, contrdto.getDistrito());
            ps.setInt(3, contrdto.getPr());
            ps.setString(4, contrdto.getTema());
            ...
            ps.setString(15, contrdto.getContratista());
            ps.setInt(16, contrdto.getUSUARIOS_idUSUARIOS());
            ps.setInt(17, contrdto.getIdBOGOTA());

            int result = ps.executeUpdate();
            if (result != 0) {
                Respuesta = "Tu Modificacion De Bogota Ha Sido Exitosa";
            } else {
                Respuesta = "Tu Modificacion De Bogota Ha Fallado ";
            }
        } catch (SQLException ex) {
            Respuesta = ex.getMessage();
            Respuesta = "Error";
...
//elimina registro bogota
    public String EliminarBogota(BogotaDTO contrdto) throws Exception {
        String Respuesta = "";
        Connection cnn = Conectar.getInstace();
        try {
            ps = cnn.prepareStatement("delete from bogota  where idBOGOTA= " + contrdto.getIdBOGOTA());
            int result = ps.executeUpdate();
            if (result != 0) {
                Respuesta = "Tu Eliminacion De Bogota Ha Sido Exitosa";
            } else {
                Respuesta = "Tu Eliminacion De Bogota Ha Fallado";
            }
        } catch (SQLException ex) {
            Respuesta = ex.getMessage();
            Respuesta = "Error";
...
//trae datos en una tabla, para hacer busqueda se pasa el parametro filtro
    public ResultSet getTabla(String filtro) throws SQLException, NamingException, Exception {
        Connection cnn = Conectar.getInstace();
        String sql = "SELECT * FROM bogota where  pr  like '%" + filtro + "%'";
        return cnn.createStatement().executeQuery(sql);
    }
//trae datos en una tabla, para hacer busqueda se pasa el parametro filtro, para hacer el conteo de las paginas se pasa el parametro page    
    public ResultSet getTabla2(String filtro, int page) throws Exception {
        ...
        String sql = "SELECT SQL_CALC_FOUND_ROWS * FROM bogota "+fil+ limit;
        return cnn.createStatement().executeQuery(sql);
    }
    //cuenta el total de registros
    public int getTotalRegistro(Boolean filtrado)throws Exception{
        String sql = "select count(central)as t from bogota";
    ...
    //verifica si el id se encentra registrado en la base de datos
    public Boolean Exists(String idBOGOTA) throws Exception{
            
        Connection cnn = Conectar.getInstace();
        ResultSet r =  cnn.createStatement().executeQuery("SELECT * FROM bogota where idBOGOTA="+idBOGOTA);
...
//lista todos los registros con un arreglo
public ArrayList<BogotaDTO> ListarTodos ()throws Exception{    
       Connection cnn = Conectar.getInstace();
       try {
            ps=cnn.prepareStatement("SELECT * FROM  bogota");
            rs = ps.executeQuery();
            while (rs.next()){
            BogotaDTO objl = new BogotaDTO();
            objl.setIdBOGOTA(rs.getInt("idBOGOTA"));

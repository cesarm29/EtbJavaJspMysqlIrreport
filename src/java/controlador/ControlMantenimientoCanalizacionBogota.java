...
import DAO.MantenimientoCanalizacionBogotaDAO;
 ...
 * @author cesar1
 */
public class ControlMantenimientoCanalizacionBogota extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            //Se genera una nueva instancia de MantenimientoCanalizacionBogotaDAO,MantenimientoCanalizacionBogotaDTO
            MantenimientoCanalizacionBogotaDAO acdao=new MantenimientoCanalizacionBogotaDAO();
            MantenimientoCanalizacionBogotaDTO actdto=new MantenimientoCanalizacionBogotaDTO();
            //se pasan los id's del formulario
            String s = request.getParameter("idMANTENIMIENTOCANALIZACIONBOGOTA");
            actdto.setIdMANTENIMIENTOCANALIZACIONBOGOTA(Integer.parseInt(request.getParameter("idMANTENIMIENTOCANALIZACIONBOGOTA")));
            ...
            //condicionales cuando se presione el boton (btnsubmit) en el formulario correspondiente a crear,modificar o eliminar va a la correspondiente accion, la genera y muestra el mensaje en pantalla y se redirecciona al jsp
                if (request.getParameter("btnSubmit") !=null && request.getParameter("btnSubmit").equals("Crear")){
                       ...
                       String salida1=acdao.CrearMantenimientoCanalizacionBogota(actdto);
                       response.sendRedirect("Tablamantenimientocanalizacionbogota.jsp?msg="+salida1);       
                }  
               else if(request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Modificar")){                 
                        String salida1=acdao.ModificarMantenimientoCanalizacionBogota(actdto);
                        response.sendRedirect("Tablamantenimientocanalizacionbogota.jsp?msg="+salida1); 

                 }      
               else if (request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Eliminar")){

                        String salida1=acdao.EliminarMantenimientoCanalizacionBogota(actdto);
                        response.sendRedirect("Tablamantenimientocanalizacionbogota.jsp?msg="+salida1); 
                }                
            }catch (Exception ex){

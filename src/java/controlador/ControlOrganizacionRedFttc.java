...
import DAO.OrganizacionRedFttcDAO;
 ...
 * @author cesar1
 */
public class ControlOrganizacionRedFttc extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            //Se genera una nueva instancia de OrganizacionRedFttcDAO,OrganizacionRedFttcDTO
            OrganizacionRedFttcDAO acdao=new OrganizacionRedFttcDAO();
            OrganizacionRedFttcDTO actdto=new OrganizacionRedFttcDTO();
            //se pasan los id's del formulario
            String s = request.getParameter("idORGANIZACIONREDFTTC");
            actdto.setIdORGANIZACIONREDFTTC(Integer.parseInt(request.getParameter("idORGANIZACIONREDFTTC")));
            ...
            //condicionales cuando se presione el boton (btnsubmit) en el formulario correspondiente a crear,modificar o eliminar va a la correspondiente accion, la genera y muestra el mensaje en pantalla y se redirecciona al jsp
                if (request.getParameter("btnSubmit") !=null && request.getParameter("btnSubmit").equals("Crear")){
                       ...
                       String salida1=acdao.crearOrganizacion(actdto);
                       response.sendRedirect("Tablaorganizacionredfttc.jsp?msg="+salida1);       
                }  
               else if(request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Modificar")){                 
                        String salida1=acdao.ModificarOrganizacion(actdto);
                        response.sendRedirect("Tablaorganizacionredfttc.jsp?msg="+salida1); 

                 }      
               else if (request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Eliminar")){

                        String salida1=acdao.EliminarOrganizacion(actdto);
                        response.sendRedirect("Tablaorganizacionredfttc.jsp?msg="+salida1); 
                }                
            }catch (Exception ex){

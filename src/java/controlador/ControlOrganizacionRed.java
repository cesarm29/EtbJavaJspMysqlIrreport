 ...
 * @author cesar1
 */
public class ControlOrganizacionRed extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            //Se genera una nueva instancia de OrganizacionRedDAO,OrganizacionRedDTO
            OrganizacionRedDAO acdao=new OrganizacionRedDAO();
            OrganizacionRedDTO actdto=new OrganizacionRedDTO();
            //se pasan los id's del formulario
            String s = request.getParameter("idORGANIZACIONRED");
            actdto.setIdORGANIZACIONRED(Integer.parseInt(request.getParameter("idORGANIZACIONRED")));
            ...
            //condicionales cuando se presione el boton (btnsubmit) en el formulario correspondiente a crear,modificar o eliminar va a la correspondiente accion, la genera y muestra el mensaje en pantalla y se redirecciona al jsp
                if (request.getParameter("btnSubmit") !=null && request.getParameter("btnSubmit").equals("Crear")){
                       ...
                       String salida1=acdao.crearOrganizacionRed(actdto);
                       response.sendRedirect("Tablaorganizacionred.jsp?msg="+salida1);       
                }  
               else if(request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Modificar")){                 
                        String salida1=acdao.ModificarOrganizacionRed(actdto);
                        response.sendRedirect("Tablaorganizacionred.jsp?msg="+salida1); 

                 }      
               else if (request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Eliminar")){

                   String salida1=acdao.EliminarOrganizacionRed(actdto);
                   response.sendRedirect("Tablaorganizacionred.jsp?msg="+salida1); 
                }                
            }catch (Exception ex){

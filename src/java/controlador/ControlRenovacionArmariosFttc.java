...
import DAO.RenovacionArmariosFttcDAO;
 ...
 * @author cesar1
 */
public class ControlRenovacionArmariosFttc extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            //Se genera una nueva instancia de RenovacionArmariosFttcDAO,RenovacionArmariosFttcDTO
            RenovacionArmariosFttcDAO acdao=new RenovacionArmariosFttcDAO();
            RenovacionArmariosFttcDTO actdto=new RenovacionArmariosFttcDTO();
            //se pasan los id's del formulario
            String s = request.getParameter("idRENOVACIONARMARIOSFTTC");
            actdto.setIdRENOVACIONARMARIOSFTTC(Integer.parseInt(request.getParameter("idRENOVACIONARMARIOSFTTC")));
            ...
            //condicionales cuando se presione el boton (btnsubmit) en el formulario correspondiente a crear,modificar o eliminar va a la correspondiente accion, la genera y muestra el mensaje en pantalla y se redirecciona al jsp
                if (request.getParameter("btnSubmit") !=null && request.getParameter("btnSubmit").equals("Crear")){                    
                        String salida1=acdao.CrearRenovacionArmariosFttc(actdto);
                        response.sendRedirect("Tablarenovacionarmariosfttc.jsp?msg="+salida1);       
                }  
               else if(request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Modificar")){                 
                        String salida1=acdao.ModificarRenovacionArmariosFttc(actdto);
                        response.sendRedirect("Tablarenovacionarmariosfttc.jsp?msg="+salida1); 
                 }      
               else if (request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Eliminar")){
                        String salida1=acdao.EliminarRenovacionArmariosFttc(actdto);
                        response.sendRedirect("Tablarenovacionarmariosfttc.jsp?msg="+salida1); 
                }                
            }catch (Exception ex){

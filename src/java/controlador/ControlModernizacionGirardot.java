...
import DAO.ModernizacionGirardotDAO;
 ...
 * @author cesar1
 */
public class ControlModernizacionGirardot extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            //Se genera una nueva instancia de ModernizacionGirardotDAO,ModernizacionGirardotDTO
            ModernizacionGirardotDAO acdao=new ModernizacionGirardotDAO();
            ModernizacionGirardotDTO actdto=new ModernizacionGirardotDTO();
            //se pasan los id's del formulario
            String s = request.getParameter("idMODERNIZACIONGIRARDOT");
            actdto.setIdMODERNIZACIONGIRARDOT(Integer.parseInt(request.getParameter("idMODERNIZACIONGIRARDOT")));
            ...
            //condicionales cuando se presione el boton (btnsubmit) en el formulario correspondiente a crear,modificar o eliminar va a la correspondiente accion, la genera y muestra el mensaje en pantalla y se redirecciona al jsp
                if (request.getParameter("btnSubmit") !=null && request.getParameter("btnSubmit").equals("Crear")){
                       ...
                       String salida1=acdao.crearModernizacionGirardot(actdto);
                       response.sendRedirect("Tablamodernizaciongirardot.jsp?msg="+salida1);       
                }  
               else if(request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Modificar")){                 
                        String salida1=acdao.ModificarModernizacionGirardot(actdto);
                        response.sendRedirect("Tablamodernizaciongirardot.jsp?msg="+salida1); 

                 }      
               else if (request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Eliminar")){

                   String salida1=acdao.EliminarModernizacionGirardot(actdto);
                   response.sendRedirect("Tablamodernizaciongirardot.jsp?msg="+salida1); 
                }                
            }catch (Exception ex){

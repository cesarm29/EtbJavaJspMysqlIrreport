 ...
 * @author cesar1
 */
public class ControlCanalizacion extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            //Se genera una nueva instancia de CanalizacionDAO,CanalizacionDTO
            CanalizacionDAO acdao=new CanalizacionDAO();
            CanalizacionDTO actdto=new CanalizacionDTO();
            //se pasan los id's del formulario
            String s = request.getParameter("idCANALIZACION");
            actdto.setIdCANALIZACION(Integer.parseInt(request.getParameter("idCANALIZACION")));
            ...
            actdto.setUSUARIOS_idUSUARIOS(Integer.parseInt(request.getParameter("USUARIOS_idUSUARIOS"))); 
                ...
                if (request.getParameter("btnSubmit") !=null && request.getParameter("btnSubmit").equals("Crear")){
                       ...
                       String salida1=acdao.crearCanalizacion(actdto);
                       response.sendRedirect("TablaCanalizacion.jsp?msg="+salida1);       
                }  
               else if(request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Modificar")){                 
                        String salida1=acdao.ModificarCanalizacion(actdto);
                        response.sendRedirect("TablaCanalizacion.jsp?msg="+salida1); 

                 }      
               else if (request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Eliminar")){

                        String salida1=acdao.EliminarCanalizacion(actdto);
                        response.sendRedirect("TablaCanalizacion.jsp?msg="+salida1); 
                }                
            }catch (Exception ex){

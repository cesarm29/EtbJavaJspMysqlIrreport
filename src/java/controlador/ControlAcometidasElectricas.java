 ...
 * @author cesar1
 */
public class ControlAcometidasElectricas extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            //Se genera una nueva instancia de AcometidasElectricasDAO,AcometidasElectricasDTO 
            AcometidasElectricasDAO acdao=new AcometidasElectricasDAO();
            AcometidasElectricasDTO actdto=new AcometidasElectricasDTO();
            //se pasan los id's del formulario
            String s = request.getParameter("idACOMETIDASELECTRICAS");
            actdto.setIdACOMETIDASELECTRICAS(Integer.parseInt(request.getParameter("idACOMETIDASELECTRICAS")));
            ...
            actdto.setUSUARIOS_idUSUARIOS(Integer.parseInt(request.getParameter("USUARIOS_idUSUARIOS"))); 
                ...
                if (request.getParameter("btnSubmit") !=null && request.getParameter("btnSubmit").equals("Crear")){
                       ...
                       String salida1=acdao.crearAcometidaElectrica(actdto);
                       response.sendRedirect("TablaAcometidaselectricas.jsp?msg="+salida1);       
                }  
               else if(request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Modificar")){                 
                        String salida1=acdao.ModificarAcometidaElectrica(actdto);
                        response.sendRedirect("TablaAcometidaselectricas.jsp?msg="+salida1); 

                 }      
               else if (request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Eliminar")){

                        String salida1=acdao.EliminarAcometidaElectrica(actdto);
                        response.sendRedirect("TablaAcometidaselectricas.jsp?msg="+salida1); 
                }
             //por si se genera error   
            }catch (Exception ex){

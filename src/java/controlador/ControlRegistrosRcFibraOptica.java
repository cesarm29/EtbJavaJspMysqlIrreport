...
import DAO.RegistrosRcFibraOpticaDAO;
 ...
 * @author cesar1
 */
public class ControlRegistrosRcFibraOptica extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            //Se genera una nueva instancia de RegistrosRcFibraOpticaDAO,RegistrosRcFibraOpticaDTO
            RegistrosRcFibraOpticaDAO acdao=new RegistrosRcFibraOpticaDAO();
            RegistrosRcFibraOpticaDTO actdto=new RegistrosRcFibraOpticaDTO();
            //se pasan los id's del formulario
            String s = request.getParameter("idREGISTROSRCFIBRAOPTICA");
            actdto.setIdREGISTROSRCFIBRAOPTICA(Integer.parseInt(request.getParameter("idREGISTROSRCFIBRAOPTICA")));
            ...
            //condicionales cuando se presione el boton (btnsubmit) en el formulario correspondiente a crear,modificar o eliminar va a la correspondiente accion, la genera y muestra el mensaje en pantalla y se redirecciona al jsp
                if (request.getParameter("btnSubmit") !=null && request.getParameter("btnSubmit").equals("Crear")){                    
                        String salida1=acdao.CrearRegistroRcFibraOptica(actdto);
                        response.sendRedirect("Tablaregistrosrcfibraoptica.jsp?msg="+salida1);       
                }  
               else if(request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Modificar")){                 
                        String salida1=acdao.ModificarRegistroRcFibraOptica(actdto);
                        response.sendRedirect("Tablaregistrosrcfibraoptica.jsp?msg="+salida1); 
                 }      
               else if (request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Eliminar")){
                        String salida1=acdao.EliminarRegistroRcFibraOptica(actdto);
                        response.sendRedirect("Tablaregistrosrcfibraoptica.jsp?msg="+salida1); 
                }                
            }catch (Exception ex){

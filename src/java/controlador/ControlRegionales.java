 ...
 * @author cesar1
 */
public class ControlRegionales extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            //Se genera una nueva instancia de RegionalesDAO,RegionalesDTO
            RegionalesDAO acdao=new RegionalesDAO();
            RegionalesDTO actdto=new RegionalesDTO();
            //se pasan los id's del formulario
            String s = request.getParameter("idREGIONALES");
            actdto.setIdREGIONALES(Integer.parseInt(request.getParameter("idREGIONALES")));
            ...
            //condicionales cuando se presione el boton (btnsubmit) en el formulario correspondiente a crear,modificar o eliminar va a la correspondiente accion, la genera y muestra el mensaje en pantalla y se redirecciona al jsp
                if (request.getParameter("btnSubmit") !=null && request.getParameter("btnSubmit").equals("Crear")){
                       ...
                       String salida1=acdao.crearRegionales(actdto);
                       response.sendRedirect("Tablaregionales.jsp?msg="+salida1);       
                }  
               else if(request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Modificar")){                 
                        String salida1=acdao.ModificarRegionales(actdto);
                        response.sendRedirect("Tablaregionales.jsp?msg="+salida1); 

                 }      
               else if (request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Eliminar")){

                        String salida1=acdao.EliminarRegionales(actdto);
                        response.sendRedirect("Tablaregionales.jsp?msg="+salida1); 
                }                
            }catch (Exception ex){

...
import DAO.RedesInternasRegionalesDAO;
 ...
 * @author cesar1
 */
public class ControlRedesInternasRegionales extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            //Se genera una nueva instancia de RedesInternasRegionalesDAO,RedesInternasRegionalesDTO
            RedesInternasRegionalesDAO acdao=new RedesInternasRegionalesDAO();
            RedesInternasRegionalesDTO actdto=new RedesInternasRegionalesDTO();
            //se pasan los id's del formulario
            String s = request.getParameter("idREDESINTERNASREGIONALES");
            actdto.setIdREDESINTERNASREGIONALES(Integer.parseInt(request.getParameter("idREDESINTERNASREGIONALES")));
            ...
            //condicionales cuando se presione el boton (btnsubmit) en el formulario correspondiente a crear,modificar o eliminar va a la correspondiente accion, la genera y muestra el mensaje en pantalla y se redirecciona al jsp
                if (request.getParameter("btnSubmit") !=null && request.getParameter("btnSubmit").equals("Crear")){                    
                        String salida1=acdao.CrearRedesInternasRegionales(actdto);
                        response.sendRedirect("Tablaredesinternasregionales.jsp?msg="+salida1);       
                }  
               else if(request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Modificar")){                 
                        String salida1=acdao.ModificarRedesInternasRegionales(actdto);
                        response.sendRedirect("Tablaredesinternasregionales.jsp?msg="+salida1); 
                 }      
               else if (request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Eliminar")){
                        String salida1=acdao.EliminarRedesInternasRegionales(actdto);
                        response.sendRedirect("Tablaredesinternasregionales.jsp?msg="+salida1); 
                }                
            }catch (Exception ex){

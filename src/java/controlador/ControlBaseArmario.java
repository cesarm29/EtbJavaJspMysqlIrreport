 ...
 * @author cesar1
 */
public class ControlBaseArmario extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            //Se genera una nueva instancia de BaseArmarioDAO,BaseDeArmarioDTO 
            BaseArmarioDAO acdao=new BaseArmarioDAO();
            BaseDeArmarioDTO actdto=new BaseDeArmarioDTO();
            //se pasan los id's del formulario
            String s = request.getParameter("idBASEDEARMARIO");
            actdto.setIdBASEDEARMARIO(Integer.parseInt(request.getParameter("idBASEDEARMARIO")));
            ...
            actdto.setUSUARIOS_idUSUARIOS(Integer.parseInt(request.getParameter("USUARIOS_idUSUARIOS"))); 
                ...
                if (request.getParameter("btnSubmit") !=null && request.getParameter("btnSubmit").equals("Crear")){
                       String salida1=acdao.crearBaseArmario(actdto);
                       response.sendRedirect("Tablabasearmario.jsp?msg="+salida1);       
                }  
               else if(request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Modificar")){                 
                        String salida1=acdao.ModificarBaseArmario(actdto);
                        response.sendRedirect("Tablabasearmario.jsp?msg="+salida1); 
                 }      
               else if (request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Eliminar")){
                        String salida1=acdao.EliminarBaseArmario(actdto);
                        response.sendRedirect("Tablabasearmario.jsp?msg="+salida1); 
                }                
            }catch (Exception ex){

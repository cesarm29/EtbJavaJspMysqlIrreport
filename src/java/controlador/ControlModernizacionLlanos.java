...
import DAO.ModernizacionLlanosDAO;
 ...
 * @author cesar1
 */
public class ControlModernizacionLlanos extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            //Se genera una nueva instancia de ModernizacionLlanosDAO,ModernizacionLlanosDTO
            ModernizacionLlanosDAO acdao=new ModernizacionLlanosDAO();
            ModernizacionLlanosDTO actdto=new ModernizacionLlanosDTO();
            //se pasan los id's del formulario
            String s = request.getParameter("idMODERNIZACIONLLANOS");
            actdto.setIdMODERNIZACIONLLANOS(Integer.parseInt(request.getParameter("idMODERNIZACIONLLANOS")));
            ...
            //condicionales cuando se presione el boton (btnsubmit) en el formulario correspondiente a crear,modificar o eliminar va a la correspondiente accion, la genera y muestra el mensaje en pantalla y se redirecciona al jsp
                if (request.getParameter("btnSubmit") !=null && request.getParameter("btnSubmit").equals("Crear")){
                       ...
                       String salida1=acdao.crearModernizacionLlanos(actdto);
                       response.sendRedirect("Tablamodernizacionllanos.jsp?msg="+salida1);       
                }  
               else if(request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Modificar")){                 
                        String salida1=acdao.ModificarModernizacionLlanos(actdto);
                        response.sendRedirect("Tablamodernizacionllanos.jsp?msg="+salida1); 

                 }      
               else if (request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Eliminar")){

                   String salida1=acdao.EliminarModernizacionLlanos(actdto);
                   response.sendRedirect("Tablamodernizacionllanos.jsp?msg="+salida1); 
                }                
            }catch (Exception ex){

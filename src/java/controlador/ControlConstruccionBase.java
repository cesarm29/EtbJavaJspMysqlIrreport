...
import DAO.ConstruccionBaseDAO;
 ...
 * @author cesar1
 */
public class ControlConstruccionBase extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            //Se genera una nueva instancia de ConstruccionBaseDAO,ConstruccionBaseDTO
            ConstruccionBaseDAO acdao=new ConstruccionBaseDAO();
            ConstruccionBaseDTO actdto=new ConstruccionBaseDTO();
            //se pasan los id's del formulario
            String s = request.getParameter("idCONSTRUCCIONBASE");
            actdto.setIdCONSTRUCCIONBASE(Integer.parseInt(request.getParameter("idCONSTRUCCIONBASE")));
            ...
            actdto.setUSUARIOS_idUSUARIOS(Integer.parseInt(request.getParameter("USUARIOS_idUSUARIOS"))); 
                ...
                if (request.getParameter("btnSubmit") !=null && request.getParameter("btnSubmit").equals("Crear")){
                       ...
                       String salida1=acdao.crearConstruccionBase(actdto);
                       response.sendRedirect("Tablaconstruccionbase.jsp?msg="+salida1);       
                }  
               else if(request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Modificar")){                 
                        String salida1=acdao.ModificarConstruccionBase(actdto);
                        response.sendRedirect("Tablaconstruccionbase.jsp?msg="+salida1); 
                 }      
               else if (request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Eliminar")){
                        String salida1=acdao.EliminarConstruccionBase(actdto);
                        response.sendRedirect("Tablaconstruccionbase.jsp?msg="+salida1); 
                }                
            }catch (Exception ex){

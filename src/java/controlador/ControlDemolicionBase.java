 ...
 * @author cesar1
 */
public class ControlDemolicionBase extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            //Se genera una nueva instancia de DemolicionBaseDAO,DemolicionBaseDTO
            DemolicionBaseDAO acdao=new DemolicionBaseDAO();
            DemolicionBaseDTO actdto=new DemolicionBaseDTO();
            //se pasan los id's del formulario
            String s = request.getParameter("idDEMOLICIONBASE");
            actdto.setIdDEMOLICIONBASE(Integer.parseInt(request.getParameter("idDEMOLICIONBASE")));
            ...
            //condicionales cuando se presione el boton (btnsubmit) en el formulario correspondiente a crear,modificar o eliminar va a la correspondiente accion, la genera y muestra el mensaje en pantalla y se redirecciona al jsp
                if (request.getParameter("btnSubmit") !=null && request.getParameter("btnSubmit").equals("Crear")){
                       ...
                       String salida1=acdao.crearDemolicionBase(actdto);
                       response.sendRedirect("Tablademolicionbase.jsp?msg="+salida1);       
                }  
               else if(request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Modificar")){                 
                        String salida1=acdao.ModificarDemolicionBase(actdto);
                        response.sendRedirect("Tablademolicionbase.jsp?msg="+salida1); 

                 }      
               else if (request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Eliminar")){

                   String salida1=acdao.EliminarDemolicionBase(actdto);
                   response.sendRedirect("Tablademolicionbase.jsp?msg="+salida1); 
                }                
            }catch (Exception ex){

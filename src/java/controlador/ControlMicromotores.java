 ...
 * @author cesar1
 */
public class ControlMicromotores extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            //Se genera una nueva instancia de MicromotoresDAO,MicromotoresDTO
            MicromotoresDAO acdao=new MicromotoresDAO();
            MicromotoresDTO actdto=new MicromotoresDTO();
            //se pasan los id's del formulario
            String s = request.getParameter("idMICROMOTORES");
            actdto.setIdMICROMOTORES(Integer.parseInt(request.getParameter("idMICROMOTORES")));
            ...
            //condicionales cuando se presione el boton (btnsubmit) en el formulario correspondiente a crear,modificar o eliminar va a la correspondiente accion, la genera y muestra el mensaje en pantalla y se redirecciona al jsp
                if (request.getParameter("btnSubmit") !=null && request.getParameter("btnSubmit").equals("Crear")){                    
                        String salida1=acdao.CrearMicromotores(actdto);
                        response.sendRedirect("Tablamicromotores.jsp?msg="+salida1);       
                }  
               else if(request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Modificar")){                 
                        String salida1=acdao.ModificarMicromotores(actdto);
                        response.sendRedirect("Tablamicromotores.jsp?msg="+salida1); 
                 }      
               else if (request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Eliminar")){
                        String salida1=acdao.EliminarMicromotores(actdto);
                        response.sendRedirect("Tablamicromotores.jsp?msg="+salida1); 
                }                
            }catch (Exception ex){

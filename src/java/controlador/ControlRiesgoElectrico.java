 ...
 * @author cesar1
 */
public class ControlRiesgoElectrico extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            //Se genera una nueva instancia de RiesgoElectricoDAO,RiesgoElectricoDTO
            RiesgoElectricoDAO acdao=new RiesgoElectricoDAO();
            RiesgoElectricoDTO actdto=new RiesgoElectricoDTO();
            //se pasan los id's del formulario
            String s = request.getParameter("idRIESGOELECTRICO");
            actdto.setIdRIESGOELECTRICO(Integer.parseInt(request.getParameter("idRIESGOELECTRICO")));
            ...
            //condicionales cuando se presione el boton (btnsubmit) en el formulario correspondiente a crear,modificar o eliminar va a la correspondiente accion, la genera y muestra el mensaje en pantalla y se redirecciona al jsp
                if (request.getParameter("btnSubmit") !=null && request.getParameter("btnSubmit").equals("Crear")){                    
                        String salida1=acdao.crearRiesgoElectrico(actdto);
                        response.sendRedirect("Tablariesgoelectrico.jsp?msg="+salida1);       
                }  
               else if(request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Modificar")){                 
                        String salida1=acdao.ModificarRiesgoElectrico(actdto);
                        response.sendRedirect("Tablariesgoelectrico.jsp?msg="+salida1); 
                 }      
               else if (request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Eliminar")){
                        String salida1=acdao.EliminarRiesgoElectrico(actdto);
                        response.sendRedirect("Tablariesgoelectrico.jsp?msg="+salida1); 
                }                
            }catch (Exception ex){

...
import DAO.ModernizacionLlanos2DAO;
 ...
 * @author cesar1
 */
public class ControlModernizacionLlanos2 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            //Se genera una nueva instancia de ModernizacionLlanos2DAO,ModernizacionLlanos2DTO    
            ModernizacionLlanos2DAO acdao=new ModernizacionLlanos2DAO();
            ModernizacionLlanos2DTO actdto=new ModernizacionLlanos2DTO();
            //se pasan los id's del formulario
            String s = request.getParameter("idMODERNIZACIONLLANOS2");
            actdto.setIdMODERNIZACIONLLANOS2(Integer.parseInt(request.getParameter("idMODERNIZACIONLLANOS2")));
            ...
            //condicionales cuando se presione el boton (btnsubmit) en el formulario correspondiente a crear,modificar o eliminar va a la correspondiente accion, la genera y muestra el mensaje en pantalla y se redirecciona al jsp
                if (request.getParameter("btnSubmit") !=null && request.getParameter("btnSubmit").equals("Crear")){
                       ...
                       String salida1=acdao.crearModernizacionLlanos2(actdto);
                       response.sendRedirect("Tablamodernizacionllanos2.jsp?msg="+salida1);       
                }  
               else if(request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Modificar")){                 
                        String salida1=acdao.ModificarModernizacionLlanos2(actdto);
                        response.sendRedirect("Tablamodernizacionllanos2.jsp?msg="+salida1); 

                 }      
               else if (request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Eliminar")){

                   String salida1=acdao.EliminarModernizacionLlanos2(actdto);
                   response.sendRedirect("Tablamodernizacionllanos2.jsp?msg="+salida1); 
                }                
            }catch (Exception ex){

 ...
 * @author cesar1
 */
public class ControlRegionalesBase extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            //Se genera una nueva instancia de RegionalesBaseDAO,RegionalesBaseDTO
            RegionalesBaseDAO acdao=new RegionalesBaseDAO();
            RegionalesBaseDTO actdto=new RegionalesBaseDTO();
            //se pasan los id's del formulario
            String s = request.getParameter("idREGIONALESBASE");
            actdto.setIdREGIONALESBASE(Integer.parseInt(request.getParameter("idREGIONALESBASE")));
            ...
            //condicionales cuando se presione el boton (btnsubmit) en el formulario correspondiente a crear,modificar o eliminar va a la correspondiente accion, la genera y muestra el mensaje en pantalla y se redirecciona al jsp
                if (request.getParameter("btnSubmit") !=null && request.getParameter("btnSubmit").equals("Crear")){
                       ...
                       String salida1=acdao.crearRegionalesBase(actdto);
                       response.sendRedirect("Tablaregionalesbase.jsp?msg="+salida1);       
                }  
               else if(request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Modificar")){                 
                        String salida1=acdao.ModificarRegionalesBase(actdto);
                        response.sendRedirect("Tablaregionalesbase.jsp?msg="+salida1); 

                 }      
               else if (request.getParameter("btnSubmit")!= null && request.getParameter("btnSubmit").equals("Eliminar")){

                        String salida1=acdao.EliminarRegionalesBase(actdto);
                        response.sendRedirect("Tablaregionalesbase.jsp?msg="+salida1); 
                }                
            }catch (Exception ex){
